...
""".lstrip(),
            ...
            self.assertFalse(config.acceptance_fault_execution_enabled)
            self.assertEqual(config.acceptance_fault_execution_run_id, "")
            self.assertEqual(config.acceptance_fault_execution_plan_sha256, "")
            ...
            self.assertEqual(config.mikan_library_fallback_scan_interval_seconds, 3600)
            self.assertEqual(config.mikan_library_fallback_scan_max_series_per_cycle, 8)
            ...
            self.assertTrue(config.pipeline_job_store_required)
            self.assertFalse(config.source_integrity_sha256_enabled)
            self.assertEqual(config.scanner_event_quiet_window_seconds, 5.0)
            self.assertEqual(config.scanner_event_stable_observations_required, 2)
            self.assertEqual(config.scanner_event_ffprobe_path, "ffprobe")
            self.assertEqual(config.scanner_event_media_probe_timeout_seconds, 30.0)
            self.assertEqual(config.scanner_event_media_probe_min_throughput_mib_per_second, 8.0)
            self.assertEqual(config.scanner_event_media_probe_max_timeout_seconds, 1800.0)
            self.assertEqual(config.scanner_event_media_probe_max_attempts, 4)
            self.assertEqual(config.scanner_event_media_probe_max_retry_seconds, 900.0)
            self.assertEqual(config.scanner_background_scan_interval_seconds, 21600)
            self.assertEqual(config.scanner_fallback_scan_interval_seconds, 21600)
            self.assertEqual(config.scanner_background_scan_startup_delay_seconds, 600)
            self.assertEqual(config.scanner_reconcile_batch_interval_seconds, 60)
